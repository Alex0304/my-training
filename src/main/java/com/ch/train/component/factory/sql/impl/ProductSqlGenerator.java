...
public enum ProductSqlGenerator implements SqlGenerator {
    ...
    QUERY_PAGE("select id,name,description,price,create_time,update_time,user_id from product", "product", "查询商品分页"),
    QUERY_COUNT("select count(1) from product", "product", "查询商品数量统计"),
    INSERT_ONE("insert into product", "product", "插入一条商品信息"),
