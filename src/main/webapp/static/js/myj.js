        ...
        "message" : function(options) {
           ...
           switch (options.type){
            ...
            var div = $('<div data-name="warMing" class="' +options.className+ '" style="min-width: 350px;text-align: center;z-index:9999999;display:none;min-height:25px;line-height:25px;position:fixed;box-shadow: 1px 2px 3px #AAAAAA;">'
                ...
                +'<div data-name="optionMsg" style="text-align: left;max-width: 1200px"  class="pull-left mLeft10"><span>'+options.msg+'</span></div></div>'
...
function baiDuTranslate(str, callback, options,from,to){
	dxmTranslate(str, callback, options,from,to);
}
...
function dxmTranslate(str, callback, options,from,to){
	var s = "";
	if(str != undefined && str != ""){
		str = $.trim(str);
		if(str.length > 5000){
	...
	case "password":
		...
		if (validateStr != '')
		{
			if (!validateStr.match(validatePassword))
			{
				...
				$('.verificode').html('密码格式不正确！不能含有空格,长度为6-20个字符！');
				return false;
			}
		}
		break;
...
(function(win){
	...
	function createElements(opts){
		...
		var overlayer = createEl('<div id="dialog_overlayer" style="height:'+h+'px;"></div>',db),
...
$(document).on('keyup','.categoryChooseOutDiv .serch input[type="text"]',function(){
	var serchVal = $.trim($(this).val().toLowerCase());
	var list = $(this).closest('.categoryChooseInDiv').find('div.categoryDiv');
    list.each(function(){
        var txtCon = strToUc($(this).find('.categoryNames').text());
        $(this).attr('txtCon',txtCon);
    });
	var serchReg = new RegExp(serchVal);
    if(serchVal){
        list.addClass('myj-hide');
        list.each(function(){
            $(this).find('.categoryNames').html(ucToStr($(this).attr('txtCon')));
            var val = $.trim($(this).find('.categoryNames').text());
            var listVal = val.toLowerCase();
            if(serchReg.test(listVal)){
                $(this).removeClass('myj-hide');
                var arr = listVal.split(serchVal);
                var otherArr = [];
                $.each(arr,function(i){
                    var str = arr[i],startNum = 0,endNum = 0;
                    if(str){
                        endNum = str.length;
                    }
                    otherArr.push({str:str,endNum:endNum})
                });
                var newStr = '',regStrNum = 0;
                $.each(otherArr,function(i){
                    var obj = otherArr[i];
                    if(obj.str){
                        newStr += val.substring(regStrNum,regStrNum + obj.endNum);
                    }
                    if(obj.str){
                        regStrNum = regStrNum + obj.endNum;
                    }
                    if((regStrNum + serchVal.length) <= val.length){
                        newStr += '<span class="fRed">'+val.substring(regStrNum,regStrNum+serchVal.length) +'</span>';
                        regStrNum += serchVal.length;
                    }
                });
                $(this).find('.categoryNames').html(newStr);
            }
        })
    }else{
...
var platformDisplayMapping = function (platformCode) {
    ...
    m['magento'] = 'Magento';
    m['magento2'] = 'Magento2';
    m['woocomm'] = 'Woocommerce';
    m['shopee'] = 'Shopee';
    m['joom'] = 'Joom';
    m['tophatter'] = 'Tophatter';
    m['walmart'] = 'Walmart';
    m['mymall'] = 'myMall';
    m['factorymkt'] = 'Factorymarket';
    m['ueeshop'] = 'Ueeshop';
    m['shopyy'] = 'Shopyy';
    m['vova'] = 'Vova';
    m['jd'] = '京东';
    return m[platformCode] == undefined ? '' : m[platformCode];
};
...
*
* */
function comOpenWinPost(url,args){
    var _form = $("<form></form>",{
...
var batchExportData = function(url,param,title,pageSize){
    ...
    $("#batchExportProductModal").modal("show");
    /*2017.2.10 byzym 二次刷新初始化*/
    $("#batchCheckReplicaProgressBarr").css("display", "none");
    setTimeout(function(){
        $("#batchCheckReplicaProgressBarr").css("width", 0+"%");
        $("#batchCheckReplicaProgressBarr").css("display", "");
    },10);
    $("#batchCheckReplicaProcessNumm").html("0");
    $("#reportShoww").empty();
    $('#upDownloadboxZ').empty();
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            checkProcessStatus(uuid, function(data){
                ...
                p = num;
                if(data.code==0){
                    $("#batchCheckReplicaProgressBarr").css("width", num + "%");
                    $("#batchCheckReplicaProcessNumm").text(data.num);
                    $("#reportShoww").text(title + "正在导出中，请稍后下载...");
                    $("#reportShowww").text("");
                    $("#upDownloadboxZu").text("");

                }else{
                    $("#batchCheckReplicaProgressBarr").css("width", "100%");
                    $("#batchCheckReplicaProgressBarr").removeClass("active");
                    $("#batchCheckReplicaProcessNumm").text(data.totalNum);
                    $("#reportShoww").empty();
                    $("#reportShowww").empty();
                    ...
                    if (data.msg) {
                        ...
                        if (urls.length > 1) {
                            ...
                            $('#upDownloadboxZ').html(spanStr);
                            $("#reportShoww").html('由于文件过大，已拆分为 '+urls.length+'个文件<br/>');
                        } else {
                            var downUrlAdder = urls[0];
                            if(downUrlAdder){
                                $("#reportShoww").html('导出文件已经生成: <span class="inline-block" style="width: 33.33%;line-height: 30px;"><a role="menuitem" tabindex="-1" href="'+downUrlAdder+'">立即下载文件</a></span>');
                            }
                        }
                    }
                }
